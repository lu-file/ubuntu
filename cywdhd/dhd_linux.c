...
#define DEFAULT_BCMDHD_FW_PATH "/vendor/modules/"
#define DEFAULT_BCMDHD_NVRAM_PATH "/vendor/modules/"
...
#define NVRAM_CYW43455 "nvram_awcm390sm.txt"
#define NVRAM_CYW4354 "nvram_azw235.txt"

#define FW_CYW43455_CLM_BLOB "cyw43455.blob"
...
bool dhd_update_fw_nv_path(dhd_info_t *dhdinfo)
{
	...
	if (cyw_cis_device == BCM43438_CHIP_ID) {
	    ...
	    sprintf(clm_path, "%s%s", DEFAULT_BCMDHD_FW_PATH, FW_CYW43455_CLM_BLOB);
	}
	DHD_ERROR(("Adding CYW firmware and NVRAM path by CIS\n"
		"\tcyw_cis_device: 0x%x\n"
		"\tfirmware path: %s\n"
		"\tclm path: %s\n"
		"\tNVRAM path:    %s\n", cyw_cis_device,fw_path,clm_path, nv_path));
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	if (!ret) {
		...
		if (ret < 0) {
			...
			ret = BCME_NOTUP;
			DHD_ERROR(("%s: ret = %d,will use wifi module's MAC\n", __FUNCTION__, ret));
		}
		memcpy(dhd->mac.octet, ea_addr.octet, ETHER_ADDR_LEN);
	} else {
		DHD_ERROR(("%s: can't get custom MAC address, ret=%d,will get default MAC\n", __FUNCTION__, ret));
	}
#endif /* GET_CUSTOM_MAC_ENABLE */
